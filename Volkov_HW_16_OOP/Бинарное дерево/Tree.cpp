...
void Tree::Save_File(const Node* node, FILE* file)const { // Save
    if (node != nullptr)
    {
        Save_File(node->m_left, file);
        fprintf(file, "%s\t", node->m_english); // output str
        fprintf(file, "%s\n", node->m_russian); // output str
        Save_File(node->m_right, file); // calling a function and passing parameters to it
    }
}

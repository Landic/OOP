...
bool LoginForUser::Login_In_System() // метод входа в систему
{
	login = "login_user.txt";
	...
	if (!login_file.is_open() && !password_file.is_open()) {
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (login_correct != login_checker || password_correct != password_checker) {
